//? maybe create tresorlkm.h for better doc
...
int
tresor_nl_cmd(struct sk_buff *skb_2, struct genl_info *info);
/* Netlink genl ops */
...
static struct genl_family tresor_nl_genl_family = {
	.hdrsize = 0,
	.name = TRESOR_NL_FAMILY_NAME,
	.version = TRESOR_NL_VERSION,
	.maxattr = TRESOR_NL_A_MAX,
	.policy = tresor_genl_policy,
	.ops = doc_exmpl_genl_ops_echo,
	.n_ops = TRESOR_NL_O_MAX,
};
...
void
printCharAsHex(const unsigned char mem[], int count)
{
	int i, k = 0;
	char hexbyte[11] = "";
	char hexline[126] = "";
	for (i = 0; i < count; i++) {		     // traverse through mem
		...
		if ((((i) % 15 == 0) && (i != 0)) || (i + 1 == count)) {
			...
			printk(KERN_INFO "tresorlkm: l%d: %s", k,
			       hexline);			 // print line to kernellog
...
int
tresor_nl_sendmsg(struct genl_info *info, struct tresor_nl_msg tresormsg)
{
	struct sk_buff *skb;
	void *msg_head;
	int rc = 0;
	...
	if (info == NULL || tresord_nl_portid == -1) {
		printk(KERN_ERR
		       "tresorlkm: tresor_nl_sendmsg: ERROR no daemon registered netlink socket\n");
	...
	/* create the message */
	msg_head = genlmsg_put(skb, 0, info->snd_seq + 1, &tresor_nl_genl_family, 0, TRESOR_NL_O_CMD);
	...
	rc |= nla_put_u32(skb, TRESOR_NL_A_OP, tresormsg.operation);
	rc |= nla_put_u32(skb, TRESOR_NL_A_DATA_LEN, tresormsg.data_len);
	rc |= nla_put_string(skb, TRESOR_NL_A_DATA, tresormsg.data);
	...
	if (rc != 0) {
		printk(KERN_INFO
		       "tresorlkm: tresor_nl_sendmsg: ERROR first genlmsg_unicast went wrong rc: %x tresord_nl_portid: %x\n",
		       rc, tresord_nl_portid);
		rc = genlmsg_unicast(&init_net, skb, info->snd_portid);
		if (rc != 0) {
			printk(KERN_INFO
			       "tresorlkm: tresor_nl_sendmsg: ERROR second genlmsg_unicast went wrong rc: %x info->snd_portid: %x\n",
			       rc, info->snd_portid);
			tresord_avail =
				0; // daemon is not reachable any longer. must be registered again.
...
tresor_nl_cmd(struct sk_buff *skb_2, struct genl_info *info)
{
	...
	attr_tresormsg.operation = nla_get_u32(info->attrs[TRESOR_NL_A_OP]);
    attr_tresormsg.data_len = nla_get_u32(info->attrs[TRESOR_NL_A_DATA_LEN]);
    memcpy(attr_tresormsg.data, nla_data(info->attrs[TRESOR_NL_A_DATA]), attr_tresormsg.data_len);

	if (attr_tresormsg.data == NULL) {
		printk(KERN_INFO "tresorlkm: tresor_nl_cmd: error while receiving data\n");
	} else {
		printk(KERN_INFO "tresor_nl_cmd: tresormsg operation: %d text_len: %s\n", attr_tresormsg.operation, attr_tresormsg.data_len);
		...
		case TRESOR_MSG_REGISTER:
			...
			tresormsg.data_len = sizeof("Hello Kernel here!");
			memcpy(tresormsg.data, "Hello Kernel here!",tresormsg.data_len);
			res = tresor_nl_sendmsg(tresord_nl_info, tresormsg);
			if (res != TRESOR_OK) {
				printk(KERN_INFO,
				       "tresorlkm: %s: ERROR cannot send register ack back because: 0x%x",
				       __FUNCTION__, res);
				break;
			}
		...
		case TRESOR_MSG_ENCRYPT:
			printk(KERN_INFO, "tresorlkm: %s: TRESOR_MSG_ENCRYPT.. text_len: %d", __FUNCTION__, attr_tresormsg.data_len);
			printCharAsHex(attr_tresormsg.data, attr_tresormsg.data_len);
			memcpy(encryption_text, attr_tresormsg.data, attr_tresormsg.data_len - 1); //! dont copy nullbyte!!
			printCharAsHex(encryption_text, AES_BLOCK_SIZE);
			complete(&completion_enc);
			break;
		case TRESOR_MSG_DECRYPT:
			printk(KERN_INFO, "tresorlkm: %s: TRESOR_MSG_DECRYPT.. text_len: %d", __FUNCTION__, attr_tresormsg.data_len);
			printCharAsHex(attr_tresormsg.data, attr_tresormsg.data_len);
			memcpy(decryption_text, attr_tresormsg.data, attr_tresormsg.data_len - 1); //! dont copy nullbyte!!
			printCharAsHex(decryption_text, AES_BLOCK_SIZE);
			complete(&completion_dec);
			break;
		default:
			printk(KERN_INFO, "tresorlkm: %s: ignore operation: %d", __FUNCTION__,
			       attr_tresormsg.operation);
			break;
		}
	}
	return TRESOR_OK;
 ...
 */
static int
tresor_crypto_setkey(struct crypto_tfm *tfm, const u8 *in_key, unsigned int key_len)
{
	int res;
	struct tresor_nl_msg tresormsg;
	res = 0;
	...
	memcpy(tresormsg.data, in_key, key_len);
	tresormsg.data_len = key_len + 1;
	tresormsg.data[tresormsg.data_len - 1] = '\0';

	res = tresor_nl_sendmsg(tresord_nl_info, tresormsg);

	if (TRESOR_OK != res) {
		printk(KERN_INFO
		       "tresorlkm: ERROR tresor_crypto_setkey: error during tresor_nl_sendmsg ret: 0x%x\n",
		       res);
 ...
 */
void
tresor_crypto_encrypt(struct crypto_tfm *tfm, u8 *dst, const u8 *src)
{
	int res, i;
	struct tresor_nl_msg tresormsg;
	...
	tresormsg.data_len = AES_BLOCK_SIZE + 1; 
	// encrypt 1 block, 16 bytes + null byte
	memcpy(tresormsg.data, src, tresormsg.data_len);
	tresormsg.data[AES_BLOCK_SIZE] = '\0'; // add nullbyte
	printCharAsHex(src, AES_BLOCK_SIZE);
	printCharAsHex(tresormsg.data, tresormsg.data_len);

	res = tresor_nl_sendmsg(tresord_nl_info, tresormsg);

	if (TRESOR_OK != res) {
		printk(KERN_INFO
		       "tresorlkm: %s ERROR during tresor_nl_sendmsg: ret: 0x%x src_addr: %x dst_addr: %x...\n",
	...
	if (0 == wait_for_completion_timeout(&completion_enc, 1000))
		printk(KERN_ERR
		       "tresorlkm: %s wait for completion timed out! src_addr: %x dst_addr: %x...\n",
 ...
 */
void
tresor_crypto_decrypt(struct crypto_tfm *tfm, u8 *dst, const u8 *src)
{
	int i, res;
	struct tresor_nl_msg tresormsg;
	//printk(KERN_INFO "tresorlkm: %s starting src_addr: %x dst_addr: %x...\n", __FUNCTION__, &src, &dst);
	...
	tresormsg.data_len = AES_BLOCK_SIZE + 1; // decrypt 1 block, 16 bytes
	//add Null byte
	memcpy(tresormsg.data, src, tresormsg.data_len);
	tresormsg.data[tresormsg.data_len - 1] = '\0';
	...
	/* used static completion, dynamic completion is also possible if completion information is propagated */
	//printk(KERN_INFO "tresorlkm: %s entering wait_for_completion..\n", __FUNCTION__);
	if (0 == wait_for_completion_timeout(&completion_dec, 1000))
		printk(KERN_ERR
		       "tresorlkm: %s wait for completion timed out! src_addr: %x dst_addr: %x...\n",
		       __FUNCTION__, &src, &dst); // uninteruruptile wait, handle with care
	//printk(KERN_INFO "tresorlkm: %s leaving wait_for_completion..\n", __FUNCTION__);
 ...
 */
static struct crypto_alg tresor_alg = { .cra_name = "tresorsgx",
...
startDaemon(void)
{
	...
	static char *envp[] = { usermodehelper_home, "TERM=linux",
...
tresor_init(void)
{
	...
	rc = genl_register_family(&tresor_nl_genl_family);

	if (rc != 0) {
		printk(KERN_ALERT "tresorlkm: %s: Error creating socket.\n", __FUNCTION__);
		genl_unregister_family(&tresor_nl_genl_family);
		return -1;
	}
...
tresor_exit(void)
{
	...
	ret = genl_unregister_family(&tresor_nl_genl_family);
